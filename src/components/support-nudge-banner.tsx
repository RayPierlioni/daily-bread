...
const dismissedKey = "next-faithful-step-support-nudge-dismissed";
...
export function SupportNudgeBanner({ completedCount }: { completedCount: number }) {
  ...
  return (
    <div className="flex flex-col gap-3 rounded-2xl border border-[#dac693] bg-[#fff7e8] p-4 text-sm leading-6 text-[#5a3e18] shadow-sm sm:flex-row sm:items-center sm:justify-between">
      <p>
        You&apos;ve completed 5 devotionals. If Next Faithful Step is helping your walk with God, consider a small gift to keep it free for people who need it.
      </p>
      <div className="flex items-center gap-2">
        <a href={supportUrl} target="_blank" rel="noreferrer" className="whitespace-nowrap rounded-lg bg-[#8c6a33] px-3 py-2 text-sm font-semibold text-white transition hover:bg-[#77592b]">
          Support the App
        </a>
